...
import com.amg.framework.cloud.grpc.strategy.AssignServerStrategy;
...
final class RoundRobinLoadBalancer extends LoadBalancer {
    ...
    private static Logger logger = LoggerFactory.getLogger(RoundRobinLoadBalancer.class);
    ...
    private final Map<EquivalentAddressGroup, Subchannel> subchannels =
            new HashMap<>();
    private final Map<EquivalentAddressGroup, Subchannel> connectsubchannels =
            new HashMap<>();
    ...
    private void updateBalancingState() {
        List<Subchannel> readySubchannels = new ArrayList<>(getSubchannels().size());
        int failure = filterNonFailingSubchannels(getSubchannels(), readySubchannels);
        if (readySubchannels.isEmpty()) {
            ...
            if (readySubchannels.size() == getSubchannels().size() - failure) {
                int startIndex = random.nextInt(readySubchannels.size());
                updateBalancingState(READY, new ReadyPicker(readySubchannels, startIndex));
            }
        }
    }
     ...
     */
    private static int filterNonFailingSubchannels(
            Collection<Subchannel> subchannels, List<Subchannel> readySubchannels) {
        int failure = 0;
    ...
    @VisibleForTesting
    final class ReadyPicker extends RoundRobinPicker {
        private  final AtomicIntegerFieldUpdater<ReadyPicker> indexUpdater =
        ...
        // 这里开始负载策略，比如参数路由、header路由
        @Override
        public PickResult pickSubchannel(PickSubchannelArgs args) {
            List<Subchannel> serverList = new ArrayList();
            try {
                AssignConfig assignConfig = SpringContextUtil.getBean(AssignConfig.class);
                if (assignConfig.isEnable() && CollectionUtils.isNotEmpty(this.list)) {
                    String serverName = this.list.get(0).asChannel().authority();
                    AssignServerStrategy assignServerStrategy = AssignServerStrategy.chooseStrategy(assignConfig, args, serverName);
                    Set<String> versions = assignServerStrategy.getVersions(args, serverName);
                    Set<String> address = assignServerStrategy.getAssignInstanceAddress(versions, serverName);
                    serverList = assignServerStrategy.getServiceSubchannel(this.list, address);
                    PickResult pickResult = assignServerStrategy.resolve(versions.stream().collect(Collectors.joining(",")), CollectionUtils.isEmpty(serverList), serverName);
                    if (pickResult != null)
                        return pickResult;
                }
            } catch (Exception e) {
            ...
            return PickResult.withSubchannel(nextSubchannel(CollectionUtils.isEmpty(serverList) ? this.list : serverList));
        }
        ...
        private Subchannel nextSubchannel(List<Subchannel> list) {
            int size = list.size();
