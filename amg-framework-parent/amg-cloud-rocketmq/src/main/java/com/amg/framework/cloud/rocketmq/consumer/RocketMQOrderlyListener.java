...
public abstract class RocketMQOrderlyListener implements MessageListenerOrderly {
    ...
    private int consumeMessageBatchSize = 10; // 默认批处理消费20条
    private int consumeMessageBatchMaxSize = 50; // 最多批处理消费50条
    private String consumerGroupSuffix = "-" + SpringContextUtil.appName.split("\\-")[1] + "Group";
    private DefaultMQPushConsumer consumer;

    public RocketMQOrderlyListener(String topic) {
        this(topic, "*", null);
    }

    public RocketMQOrderlyListener(String topic, String consumerGroup, int consumeMessageBatchSize) {
        this(topic, "*", consumerGroup, consumeMessageBatchSize);
    }

    public RocketMQOrderlyListener(String topic, int consumeMessageBatchSize) {
        this(topic, "*", null, consumeMessageBatchSize);
    }

    public RocketMQOrderlyListener(String topic, String consumerGroup) {
        this(topic, "*", consumerGroup);
    }

    public RocketMQOrderlyListener(String topic, String tag, String consumerGroup) {
        this.topic = topic;
        this.tag = tag;
        this.consumerGroup = consumerGroup;
    }

    public RocketMQOrderlyListener(String topic, String tag, String consumerGroup, int consumeMessageBatchSize) {
        this.topic = topic;
        this.tag = tag;
        this.consumerGroup = consumerGroup;
